...
有原始素材时先做「零读前知识测试」：逐段列出素材的时间线、主要参与者、数字口径与反证，写进按章连贯的正文；找没有读过素材的人只读手册，看是否能重述事件的来龙去脉。卡片/算例/目录不能替代正文。检查每个解释性段落是否先交代一句总结，再展开材料并只划出少量关键事实或口径；手机窄屏不能为了排版牺牲原有信息；逐行对照表需检查 320px 阅读顺序与列名。没有数据支撑时不要为了凑图而配图；没有必要的计算输入时不要编人物与交易。完成这一步后再运行下述工程清单。
